...
class NaukriClient:
    ...
    page navigation, automation flags removal, and explicit waits.
    """
    ...
    def start_driver(self) -> webdriver.Chrome:
        ...
        try:
            ...
            # Prevent Google from blocking login with "This browser or app may not be secure"
            ...
            logger.info("Selenium Chrome WebDriver started successfully with persistent session.")
            return self._driver
        except Exception as exc:
            logger.error(f"Failed to start Selenium WebDriver: {exc}")
            raise SeleniumDriverError(
                f"Could not start Chrome WebDriver: {str(exc)}"
            )

    @property
    def driver(self) -> webdriver.Chrome:
        """Returns the active driver or starts a new instance."""
    ...
    def wait(self) -> WebDriverWait:
        """Returns the active WebDriverWait instance."""
